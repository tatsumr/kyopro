...
- 例えば $[2, 5)=1, [8, 10)=3$ のときに `erase(4, 9, add, del)` を呼ぶと $[2, 4)=1, [9, 10)=3$ となる。処理の中で `del(2, 5, 1)`、`del(8, 10, 3)`、`add(2, 4, 1)`、`add(9, 10 3)` が呼ばれる。

### size
```cpp
int size()
```
set に入っている区間の個数を返す（実装上の都合で番兵を 2 つ入れているが `size()` には含まれない）。
